...
function cup_java_build {
    ...
    ## LOGIC
    javac -cp "java_cup.jar:java_cup_runtime.jar:jflex-full-1.8.2.jar:" find . -iname "*.java" | tr "\n" " "
...
do
    ...
    case $technology in
        'bison-c++')
            bison_cpp_produce "$source_filename" "$output_asm"
            ;;
        'cup-java')
            cup_java_produce "$source_filename" "$output_asm"
            ;;
        'lark-python')
            lark_python_produce "$source_filename" "$output_asm"
            ;;
        'pgen-c++')
            pgen_cpp_produce "$source_filename" "$output_asm"
            ;;
        'pgen-java')
            pgen_java_produce "$source_filename" "$output_asm"
            ;;
        'pgen-python')
            pgen_python_produce "$source_filename" "$output_asm"
            ;;
    esac
    ...
    if [[ $? == 0 ]]; then
        ...
        spim -nomapped_io -q -a -f "$OUTPUT_DIRECTORY$output_asm" < "$TEST_DIRECTORY$input_filename" > "$OUTPUT_DIRECTORY$output_filename" 2>&2
